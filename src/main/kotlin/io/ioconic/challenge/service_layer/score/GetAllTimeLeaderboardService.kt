...
@Service
class GetAllTimeLeaderboardService(private val scoreRepository: ScoreRepository) {
    fun execute(): List<Score>{
        return scoreRepository.findAll(PageRequest.of(0, 10).withSort(Sort.by(Sort.Direction.DESC, "score"))).toList()
    }
}
